...
- `liquidation_revision_audit` — before/after snapshots of applied (and rejected) edits to submitted liquidations (Jul 2026 edit-approval workflow: `POST /api/liquidations/:id/propose-edit` by owner/admin, `/revision/approve|reject` superadmin-only, proposal stored in `pending_revision` on the liquidation doc; apply reconciles CA balance delta + synced `project_expenses` rows). See `docs/PROJECT_HISTORY.md` 2026-07-02.
...
12. **Duplicate default user records**: as of 2026-07-02, only `projects` is still duplicated in `users` — two identical docs (`fSo0DISMnjbFi1GogMdl` updated 2026-04-02, `user_21` updated 2026-03-05; same password hash/email/name, zero FK references to either id in `liquidations`/`cash_advances`/`project_expenses`/etc). `admin` (`user_24`) is now a single doc — no longer duplicated. This is why editing the `projects` account's password in User Management throws "Username already exists": the PATCH uniqueness check is correctly detecting the real second doc, not misbehaving. Login resolves to `fSo0DISMnjbFi1GogMdl` today (most-recently-updated wins when multiple docs share a matching password). RJ chose to hold off on deleting the stale `user_21` — confirm with him before removing it. `TJC` and `RJR` were confirmed non-duplicated in the 2026-05-25 read-only audit.
...
17. **LQ-0023 internal duplicate rows** (liquidation `liq_10`, Renzel): "Gasoline - Kim Solis" ₱1,000 and "Toll RFID" ₱182 (both 2026-02-28) each appear twice — ₱1,182 double-entered. Fix via the new edit-approval flow or a direct patch (the 2026-07-02 LQ-0022 typo fix in `docs/PROJECT_HISTORY.md` is the template). The 2026-07-02 liquidation edit-approval workflow itself is **not runtime-verified** yet.
18. **Payroll module (`src/components/payroll/*.tsx`) does not follow `docs/DESIGN_PHILOSOPHY.md`** — flagged by RJ 2026-07-02, deliberately deferred (a dedicated redesign session, not a quick fix). Concrete gaps found in `PayrollDashboard.tsx`, `PayrollRegister.tsx`, `EmployeeList.tsx`, `GovernmentContribTable.tsx`: plain `Box` root (no `height:'100%', overflow:'hidden'`); page title is `h5`/`Typography variant="h5"` not `h4`; KPI cards are plain `Paper` with `borderLeft` accent instead of gradient `Card` + `NET_PACIFIC_COLORS`; table headers use a hardcoded dark navy `bgcolor: '#2c3242'` + white text instead of the standard light `stickyHeader` style; hardcoded hex colors (`#2853c0`, `#4caf50`, etc.) throughout instead of the `NET_PACIFIC_COLORS` constant. Table **sorting was added** to `PayrollRegister.tsx` and `EmployeeList.tsx` in this session (via `TableSortLabel`, with an explicit white-color override since `TableSortLabel` doesn't inherit the dark header's text color) — that part is done independent of the broader redesign. Next session: rebuild these pages against `Dashboard.tsx` as the reference implementation per DESIGN_PHILOSOPHY.md §14.
...
19. **Liquidation receipt sync/attachments/rotation (2026-07-16, on `rj/dev`, commit `209d94d`)** — see `docs/PROJECT_HISTORY.md` "Receipt linking, PDF attachments, expense pagination, rotatable/persistent receipt viewer" for full detail. Summary: (a) `receiptRef` now carried through the liquidation→`project_expenses` bulk-insert sync path (was silently dropped) and through revision-approval (`applyLiquidationRevision`); new admin-callable `POST /api/project-expenses/backfill-liquidation-receipts` (also fired by the "Sync from Liquidation" button in Expense Monitoring) retroactively links older already-synced rows — **RJ needs to click that button once against prod** to backfill historicals, not yet done. (b) the "attach receipt" input in Expense Monitoring now accepts `.pdf`/`.heic`/`.heif` (was image-only); the camera/OCR "Scan" flow stays image-only by design (crop tool can't take a PDF). (c) Expense Monitoring's "Recent Expenses" table replaced its hard `.slice(0, 50)` cap with real `TablePagination`. (d) new shared `src/components/ReceiptViewer.tsx` (draggable pane, auto-sizes to the receipt's aspect ratio including at 90°/270° rotation) used by Expense Monitoring, `TaxFilerLedgerPage.tsx`, and — newly, it had none before — `LiquidationFormPage.tsx`. Rotation persists by baking it into real pixels (canvas) and overwriting the same OneDrive item in place via new `PUT /api/onedrive/item/:id/content` (id/webUrl unchanged, so no Firestore write is needed) — this deliberately works even on submitted/locked liquidations since it never touches `rows_json`/`receipts_json`/totals or the edit-approval workflow. Verified end-to-end in-browser against real prod data (rotate+save+reopen on an actual liquidation receipt); `tsc`/CI build clean. Not yet pushed to `main`.
