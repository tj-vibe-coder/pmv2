...
14. **Finance module integration roadmap** (started 2026-06-25): unify Liquidation/Reimbursement/Investment Tracker/Finance Home/Project Expense Report/Expense Monitoring around a new Firestore `project_expenses` collection. **Phase 0 DONE** (financeCategories.ts, project_expenses API + indexes, Expense Monitoring migrate-to-cloud banner). **Phases 1–3 pending** — full roadmap + per-phase scope in `docs/PROJECT_HISTORY.md` ("Finance module integration roadmap"). Note: `firestore.indexes.json` has 3 new `project_expenses` indexes that are **not yet deployed** (`firebase deploy --only firestore:indexes`).
