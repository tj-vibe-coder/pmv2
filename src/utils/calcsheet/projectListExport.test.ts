...
test('workbook has a Projects sheet with header + one row per project', () => {
  ...
  expect(ws.getRow(1).getCell(11).value).toBe('Created By');
  ...
  expect(ws.getRow(2).getCell(11).value).toBe('TJ Caballero');
  expect(ws.getRow(3).getCell(11).value || null).toBeNull(); // '' or null both mean "no creator"
});
...
test('Updated Status data cells start blank and carry the status dropdown', () => {
  ...
  for (const r of [2, 3]) {
    ...
    expect(cell.dataValidation).toMatchObject({
      type: 'list',
      allowBlank: true,
      formulae: ['"draft,for_review,sent,won,lost,inactive"'],
    });
  }
});

test('header row is bold, frozen, and autofiltered across all 11 columns', () => {
  const ws = buildProjectListWorkbook(rows).getWorksheet('Projects')!;
  expect(ws.getRow(1).font).toMatchObject({ bold: true });
  expect(ws.views[0]).toMatchObject({ state: 'frozen', ySplit: 1 });
  expect(ws.autoFilter).toBe('A1:K1');
});
